...
if proxy_host.present? && proxy_port.present?
  ...
  Rails.logger.debug(
    ...
    "(auth=#{proxy_user.present?})"
      ...
      # Аксессуары нужно извлекать сразу после открытия их sheet/modal.
      # Ниже мы можем открыть measurements sheet, и тогда финальный browser.body
      # уже будет содержать не upsell-модалку, а модалку замеров.
      accessories_related = []
      accessories_opened = false

      begin
        require "timeout"
        accessories_opened = Timeout.timeout(ENV.fetch("PIPF_ACCESSORIES_TIMEOUT", "20").to_i) do
          try_open_accessories_modal!(browser)
        end
      rescue Timeout::Error
        Rails.logger.warn "PlDetailsFetcher.fetch_modal_with_headless_browser: accessories modal opening timed out"
      rescue => e
        Rails.logger.debug "PlDetailsFetcher.fetch_modal_with_headless_browser: accessories modal opening failed: #{e.class} - #{e.message}"
      end

      if accessories_opened
...
measurements_opened =
  if respond_to?(:try_open_measurements_sheet!, true)
    ...
    false
      ...
      # Добираем related из финального DOM тоже: иногда upsell уже присутствует в SSR,
      # а иногда был открыт до measurements и сохранён выше в accessories_related.
      accessories_related = (Array(accessories_related) +
        extract_related_products_from_accessories_modal(modal_doc) +
        extract_related_products_from_accessories_grid(modal_doc)).map(&:to_s).uniq
  ...
  def close_pipf_dialogs!(browser)
    browser.evaluate(<<~'JS')
      (function() {
        const selectors = [
          ...
          'button[aria-label*="Uždaryti" i]',
          '.pipf-modal-header__close',
          '.pipf-sheet__close',
          '[class*="modal"] button[class*="close"]',
          '[class*="sheet"] button[class*="close"]'
        ];
    ...
    Rails.logger.debug "PlDetailsFetcher.close_pipf_dialogs!: #{e.message}"
    false
  end
...
def try_open_accessories_modal!(browser)
  ...
  [0.0, 0.45, 0.85].each do |scroll_point|
  ...
  def accessories_modal_clickable?(doc)
    ...
    doc.css("button, a, [role='button'], [class*='accessories'], [class*='upsell']").any? do |node|
      txt = node.text.to_s.downcase
      aria = node["aria-label"].to_s.downcase
      testid = node["data-testid"].to_s.downcase
      cls = node["class"].to_s.downcase
      id = node["id"].to_s.downcase
      haystack = [txt, aria, testid, cls, id].join(" ")

      haystack.include?("pokaż wszystkie akcesoria") ||
        haystack.include?("akcesoria") ||
        haystack.include?("akcesor") ||
        haystack.include?("аксессуар") ||
        haystack.include?("аксесуар") ||
