...
module CategoryHierarchyImport
  class Importer
    ...
    def initialize(file_path:, dry_run: true, soft_delete_missing: false, purge_deleted_assets: false,
      ...
      purge_misplaced_visuals: true, conflict_resolutions: {},
      report_dir: DEFAULT_REPORT_DIR, logger: Rails.logger)
      ...
      @report_payload = {
        ...
        purge_misplaced_visuals: @purge_misplaced_visuals,
        validations: {
          conflict_resolutions_count: @conflict_resolutions.size
        },
        changes: {
    ...
    def apply_conflict_resolution!(existing, incoming)
      ikea_id = incoming[:ikea_id].to_s
      resolution = @conflict_resolutions[ikea_id]
      ...
      matched_candidate =
        candidates.find do |candidate|
          candidate[:parent_ids].map(&:to_s) == resolution[:parent_ids] &&
            candidate[:translated_name].to_s.strip == resolution[:translated_name]
        end
    
      base = matched_candidate || incoming
    
      resolved = base.merge(
        parent_ids: resolution[:parent_ids],
        translated_name: resolution[:translated_name]
      )
    
      if resolution[:original_name].present?
        resolved[:original_name] = resolution[:original_name]
      end
    
      resolved[:donor_ikea_ids] =
        (Array(existing[:donor_ikea_ids]) + Array(incoming[:donor_ikea_ids]) + [ikea_id]).map(&:to_s).uniq
    
      resolved[:source_occurrence_count] =
        existing[:source_occurrence_count].to_i + incoming[:source_occurrence_count].to_i
    ...
    def normalize_conflict_resolutions(hash)
      (hash || {}).each_with_object({}) do |(ikea_id, attrs), acc|
        attrs = attrs.to_h.symbolize_keys
    
        acc[ikea_id.to_s] = {
          parent_ids: Array(attrs[:parent_ids]).map(&:to_s),
          translated_name: attrs[:translated_name].to_s.strip,
          original_name: attrs[:original_name].presence&.to_s&.strip
        }
      end
    end
    ...
    def walk_nodes(nodes, ancestor_ids, desired, seen_rows)
      Array(nodes).each do |node|
        ...
        if existing && state_conflict?(existing, attrs)
          resolved = apply_conflict_resolution!(existing, attrs)
        
          if resolved
            desired[ikea_id] = resolved
          ...
          desired[ikea_id] = merge_same_state!(existing, attrs)
        else
          desired[ikea_id] = attrs
        end

        current_state = desired[ikea_id]
        next_ancestor_ids = current_state[:parent_ids].map(&:to_s) + [ikea_id.to_s]

        walk_nodes(node[:children], next_ancestor_ids, desired, seen_rows)
      end
    end

    def merge_same_state!(existing, incoming)
      existing.merge(
        donor_ikea_ids: (Array(existing[:donor_ikea_ids]) + Array(incoming[:donor_ikea_ids]) + [existing[:ikea_id]]).map(&:to_s).uniq,
        source_occurrence_count: existing[:source_occurrence_count].to_i + incoming[:source_occurrence_count].to_i
      )
    end

    def state_conflict?(a, b)
      a[:parent_ids].map(&:to_s) != b[:parent_ids].map(&:to_s) ||
        a[:translated_name].to_s.strip != b[:translated_name].to_s.strip
    end
