#!/usr/bin/env ruby
# Collapse duplicate try_open_accessories_modal! methods into one synchronous version.
#
# Usage from Rails project root:
#   ruby -c fix_pipf_accessories_collapse_duplicate_methods.rb
#   ruby fix_pipf_accessories_collapse_duplicate_methods.rb
...
defs = src.enum_for(:scan, /def try_open_accessories_modal!\(browser\)/).map { Regexp.last_match.begin(0) }
abort "try_open_accessories_modal! not found" if defs.empty?

start_idx = defs.first
end_idx = src.index(/\n\s*def accessories_modal_clickable\?\(doc\)/, start_idx)
abort "def accessories_modal_clickable?(doc) not found after try_open_accessories_modal!" unless end_idx
...
replacement = new_method + src[end_idx, 1] # preserve the newline before next def if present
src = src[0...start_idx] + new_method + src[end_idx..]

backup = "#{path}.bak_collapse_accessories_methods_#{Time.now.strftime('%Y%m%d%H%M%S')}"
File.write(backup, original)
File.write(path, src)

puts "Patched #{path}"
puts "Backup: #{backup}"
puts "Removed duplicate methods: #{defs.size}"
puts
puts "Check:"
puts "  ruby -c app/lib/pl_details_fetcher.rb"
puts "  grep -n 'def try_open_accessories_modal\\|sync accessories candidates clicker\\|opened=false tried\\|opened=true clicked\\|def accessories_modal_clickable' app/lib/pl_details_fetcher.rb"
