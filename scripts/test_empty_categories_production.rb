#!/usr/bin/env ruby
# Скрипт для тестирования категорий без продуктов на проде
...
puts '=' * 80
puts 'Тестирование категорий без продуктов на проде'
puts '=' * 80
puts "Всего категорий без продуктов: #{Category.left_joins(:products).where(products: { id: nil }).where(is_deleted: [false, nil]).count}"
puts '=' * 80
...
test_categories.each_with_index do |cat, index|
  ...
  begin
    ...
    results[:api_by_id_failed] << { id: cat.ikea_id, name: cat.name, error: e.message[0..100] }
...
if results[:html_parsing_failed].any?
  puts "\n❌ HTML парсинг не сработал для:"
  results[:html_parsing_failed].first(5).each do |item|
    puts "  - #{item[:id]} | #{item[:name]}"
    puts "    URL: #{item[:url]}" if item[:url]
    puts "    Ошибка: #{item[:error]}" if item[:error]
  end
end

if results[:api_by_id_failed].any?
  puts "\n❌ API по ID не сработал для:"
  results[:api_by_id_failed].first(5).each do |item|
    puts "  - #{item[:id]} | #{item[:name]}"
    puts "    Ошибка: #{item[:error]}" if item[:error]
  end
end
