#!/usr/bin/env ruby
# Robust PIPF accessories clicker for PlDetailsFetcher.
#
# Fixes two things:
# 1) try_open_accessories_modal! tries all visible accessory candidates one-by-one,
#    not just the first match.
# 2) headless extraction also reads accessory grids, not only .pipf-upsell-modal.
#
# Usage from Rails project root:
#   ruby -c fix_pipf_accessories_try_all_candidates.rb
#   ruby fix_pipf_accessories_try_all_candidates.rb
...
if src.include?("try_open_accessories_modal!: trying all visible accessories candidates")
  puts "Already patched: robust accessories candidate loop is present."
  exit 0
end

# 1) Replace try_open_accessories_modal! method.
...
new_method = <<~'RUBY'
  def try_open_accessories_modal!(browser)
    ...
    # Close product-details / included sheet before looking for accessories.
    begin
      browser.evaluate(<<~'JS')
        (function() {
          const isVisible = (el) => {
    ...
    result = browser.evaluate(<<~'JS')
      async function() {
        ...
        const badRe = /informacje o produkcie|podrobna informacja|подробная информация|product details|measurements|wymiary|размер/i;

        const hasAccessories = () => {
          const modal = document.querySelector(".pipf-upsell-modal");
          const modalSkus = document.querySelectorAll(".pipf-upsell-modal [data-product-number]").length;
          const gridSkus = document.querySelectorAll("[class*='upsell'] [data-product-number], [class*='accessories'] [data-product-number]").length;
          return !!modal || modalSkus > 0 || gridSkus > 0;
        };
        ...
        const collect = () => {
          ...
          return Array.from(document.querySelectorAll(selectors)).filter((el) => {
            ...
            // Prefer actual clickable nodes; for wrappers, try to use their inner button/link later.
...
# 2) Make headless extraction include accessory grids too.
old_extract = <<~'RUBY'
...
backup = "#{path}.bak_accessories_try_all_#{Time.now.strftime('%Y%m%d%H%M%S')}"
