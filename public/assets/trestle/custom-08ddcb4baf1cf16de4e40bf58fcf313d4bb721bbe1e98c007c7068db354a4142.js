...
(function() {
  ...
  function initBuilders() {
    document.querySelectorAll(BUILDER_SELECTOR).forEach(container => {
      if (!builderInstances.has(container)) {
    ...
    initTinyMCE() {
      ...
      this.blockList.querySelectorAll('.content-article-block-content').forEach(el => {
        ...
        tinymce.init({
          ...
          toolbar: 'undo redo | formatselect | bold | bullist numlist | link code',
          block_formats: 'Текст=p; Заголовок=h2;',
          content_style: 'body { font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif; font-size: 14px; } h2 { font-size: 1.5rem; font-weight: bold; }',
